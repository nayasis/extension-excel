...
public class ApachePoiReader {
	...
	public NList readSheet( InputStream stream, boolean readHeader ) throws UncheckedIOException {
		...
		read( stream, workbook -> list[0] = read( workbook, firstSheetIndex(workbook), readHeader ) );
		return list[0];
	}

	public NList readSheet( InputStream stream, String sheetName, boolean readHeader ) throws UncheckedIOException {
		NList[] list = new NList[1];
		read( stream, workbook -> list[0] = read( workbook, getSheetNames(workbook).get(sheetName), readHeader ) );
		return list[0];
	}
	...
	public Map<String,NList> readAll( InputStream stream, boolean readHeader ) throws UncheckedIOException {
		...
		read( stream, workbook -> getSheetNames( workbook ).forEach( (name,index) -> result.put( name, read(workbook,index,readHeader) ) ) );
		return result;
	}
    ...
    private Object getNumericCellValue( Cell cell ) {
		...
		if( isCellDateFormatted(cell) ) {
			...
			return new CellDateFormatter(dateFormat).format( DateUtil.getJavaDate(val) );
